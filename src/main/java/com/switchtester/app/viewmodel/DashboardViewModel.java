    ...
    public class DashboardViewModel implements Initializable {
        ...
        // Keep a reference to the currently loaded controller if it needs cleanup
        private Object currentLoadedController;
        ...
        private void showScreen(String fxmlPath, String title) {
            // Clean up the previous controller if it implements a cleanup method
            if (currentLoadedController instanceof DebugViewModel) {
                ((DebugViewModel) currentLoadedController).cleanup();
                ApplicationLauncher.logger.info("Called cleanup on previous DebugViewModel.");
            }
            // Add similar checks for other ViewModels that might need cleanup

            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource(fxmlPath));
                Parent screen = loader.load();

                // Get the controller and store it for potential future cleanup
                Object controller = loader.getController();
                currentLoadedController = controller; // Store the current controller
        ...
        private void updateActiveNavItem(String activeTitle) {
            ...
            // Special handling for Settings and its sub-views
                       ...
                       activeTitle.equalsIgnoreCase("Arduino Opta Configuration")) { // Added Arduino Opta Config
        ...
        private void loadSettingsViewDirectly() {
            try {
                ...
                // Store the settings controller for potential cleanup or further interaction
                Object controller = settingsLoader.getController();
                ...
                SettingsViewModel settingsController = (SettingsViewModel) controller;
